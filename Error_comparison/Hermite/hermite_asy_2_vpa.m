function [x] = hermite_asy_2_vpa(n)
...
% initial guess and functions evaluation are done with Asymptotic formula 
    ...
    for it =1:20
        ...
        if max(abs(dthe)) < vpa(sqrt(eps)/10)
            break;
        end
    end

    x_sym = sqrt(2*n+1).*cos(the);
    x = vpa(x_sym, 32);
end
...
function x_in = initialguess(n)
    ...
    airyrts = -T(3 * vpa(pi) / 8 * (4 * (1:nh) - 1));

    airy_roots = vpa([-2.33810741045976703848919725244674, -4.08794944413097061663698870145739, -5.52055982809555105912985551293129, ...
    ...
    init = (vpa(pi)/2) * ones(1, double(nh));
    cons2 = ((4 * nh + 3) - 4 * (1:nh)) / mu * vpa(pi);
    for k = 1:7
    ...
    p = 0.4985 + eps;
    x_in = [x_sin(1:floor(p * double(n))), x_airy(ceil(p * double(n)):end)];
