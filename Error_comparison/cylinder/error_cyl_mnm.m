function [x_value, y_value] = error_cyl_mnm(mu_input,alp)
   ...
   while x1 > s && i < 3
    ...
    f2=@(x) besselj(mu-1,x)*cos(alp)-bessely(mu-1,x)*sin(alp);
    h_vpa=@(x) f1(x)/f2(x);
    b_vpa = @(x) (h_vpa(x) ./ abs(h_vpa(x))) .* min(1, abs(h_vpa(x)));
